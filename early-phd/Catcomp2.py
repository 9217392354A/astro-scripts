...
for line in firstFile.readlines():
    ...
    if line[0] == "r" or line[0] == "D" or line[0] == ",":
        continue
    ...
    # save info
    count += 1
    master[count] = {"ra":double(info[0]), "raErr":double(info[1]), "dec":double(info[2]), "decErr":double(info[3]),\
                     "flux":double(info[4]), "fluxErr":double(info[5])}
