...
n=50 #number of interations of monty carlo
n_bins = 25 #number of radial bins
...
for i in range(0,n):
	...
	new_cat = x_matcher(rand_cat, ['ra_rand', 'dec_rand'], catA.where((catA.f250 / catA.e250) >= 3.0), ['ra', 'dec'], r_search )
...
if False: 
	...
	xx = binedges[:-1]+r_search*3600.0/n_bins
...
sub1.bar(binedges[:-1], mine_hist, width=r_search*3600.0/n_bins, fc ='None',hatch='/', yerr=np.sqrt(mine_hist.astype(np.float)))
sub1.tick_params(axis='x', labelbottom='off')
sub1.set_ylabel('Number of galaxies')

sub2.bar(xx, yy, width=r_search*3600.0/n_bins, fc ='b', yerr=yy_error)
...
fig.savefig('/Users/chrisfuller/Desktop/angular_contamination'+ '-r_search_' + str(np.around(r_search)) + '-nbins_' + str(n_bins) + '-inter_' + str(n) + '-contamination5---'+ str(np.around(contam)) +'.pdf', dpi=600.0)
finished()
