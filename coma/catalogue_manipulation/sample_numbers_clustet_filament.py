...
for i in range(len(cats)):
	...
	for j in range(3):
		t = cluster.where(cluster[types[j]] == 1)
		nTypes.append(len(t))
	...
	cluster_line = name + ' & ' + str(nTypes[0]) + '& ' + str(nTypes[1])+ '&' + str(nTypes[2]) + '&' 
	...
	for j in range(3):
		t = filament.where(filament[types[j]] == 1)
		nTypes.append(len(t))
	...
	filament_line = str(nTypes[0]) + '& ' + str(nTypes[1])+ '&' + str(nTypes[2]) + '\\\\'

	print cluster_line + filament_line
