...
if False: 
	paras = [['DMASS'], ['DMASS', 'SMASS']]#, ['SMASS']]
	ylabs = ['$Log_{10} (M_{dust} / $M$_{\odot}$)', '$Log_{10} (M_{dust} / M_{star}$)']#, '$Log_{10}(M_{star}/$M$_{\odot})$']
	ylims = [[5.5, 9.1], [-4.8, -1.4 ]]#, [6.0, 12.]]
...
if False:
	paras = [ ['sSFR'], ['METAL'],['g','r']]
	ylabs = [ '$Log_{10} (sSFR)$', '$Log_{10} (O/H)$', 'g-r']
	ylims = [[-13.2,-9.1], [7.9, 9.5], [0.1,0.95]]	

if False:#full columns
	paras = [['DMASS_250', 'SMASS'], ['SRF'], ['sSFR'], ['GMASS', 'SMASS'], ['METAL'],['g','r'], ['SMASS']]
...
bins_density = [0.0, 1., 7.0]

#create figure and subplots
fig, subs = plt.subplots(nrows=len(bins_density)-1, ncols=len(paras), sharex=False, sharey=True, squeeze=True, figsize = (3.0*len(paras),2.*len(bins_density)-1), facecolor='w',edgecolor='w')
...
for cluster_i in range(1):
	...
	for j in range(len(paras)):
		...
		for i in range(len(bins_density)-1):
			...
			histo(y=y_inter[(x_inter > lower) & (x_inter < upper)], bin_lims=bins_params, subplot = sub, colour = 'g', style = 'o', a1= 0.3, a2= 1.)
			histo(y=y_late [(x_late  > lower) & (x_late  < upper)], bin_lims=bins_params, subplot = sub, colour = 'b', style = '/', a1 = 0.3 , a2= 1.  )
			histo(y=y_early[(x_early > lower) & (x_early < upper)], bin_lims=bins_params, subplot = sub, colour = 'r', style = '\\', a1 = 0.3 , a2= 1.   )
			...
			sub.set_xlim(ylims[j][0], ylims[j][1])

			
##### plotting options
plt.subplots_adjust(left=0.11, bottom=0.1, right=0.98, top=0.99, wspace=0., hspace=0.0)



fig.savefig('/Users/chrisfuller/Desktop/trends_density_DUST-twobin_all.pdf')
plt.show()