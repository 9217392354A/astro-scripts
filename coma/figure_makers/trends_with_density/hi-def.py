...
cats = [[detected,undetected], [detected,undetected], [detected,detected]]
...
if True:
	...
	ycols = ['G2S', 'G2D']
	ylabs = ['$\log_{10} (M_{gas} / M_{stars}$)' ,'$\log_{10} (M_{gas} / M_{dust}$)']
	ylims = [[-1.65,0.8],[1.2, 3.1],[-0.2,0.8]]
	

fig, subs = plt.subplots(nrows=2, ncols=len(xcols), sharex=False, sharey=False, squeeze=False, figsize = (8., 4.5), facecolor='w',edgecolor='w')
		...
		for ll in range(2):
			...
			for k in range(len(types)):
				...
				if ll == 0:
					...
					sub.scatter(x, y, marker='o', s=y_n*2.0, c=colours[k], edgecolor=colours[k], alpha=0.35)
				else:
					sub.errorbar(x, y, yerr=y_err, c=colours[k], ls='', alpha=0.35)
					sub.scatter(x, y, marker='o', s=y_n*2.0, facecolor='None', edgecolor=colours[k], alpha=0.35)
				...
				if i!= 3:
					sub.set_xlim(-1.4, 3.6)
					#subs[j,i].invert_xaxis()

				if i == 3:
					sub.set_xlim(-1.4,1.5)
				...
				if i != 0: sub.tick_params(axis = 'y', labelleft = 'off')

				sub.set_ylim(ylims[j][0], ylims[j][1])
printTable(coeffs_dict)

##### plotting options
plt.subplots_adjust(left=0.10, bottom=0.12, right=0.97, top=0.97, wspace=0.0, hspace=0.0)
#fig.savefig('/Users/chrisfuller/Dropbox/phd/papers/coma/hidef-density.pdf')
plt.show()
