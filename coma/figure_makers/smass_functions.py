...
cat = cat.where(cat.SMASS >= 9.0)
#cat = cat.where(cat.r <= 17.6)
...
#volumes= [1.0, 1.0]
#cats  
cats = [cat.where(cat.RADIUS_VIR <= 1.0), cat.where(cat.RADIUS_VIR > 1.0), [-1.2,9.5E10,0.002], [-1.2,19.2E10,0.3]]
...
def lmfitter(x , y, y_error):
	...
	params.add('LStar', value=19.2E10, vary=True)
	params.add('alpha', value=-1.2, vary=True)
...
def printFit(params):
	...
	try:
		...
		myPrint( 'LStar:', LStar/10E7, alphaErr/10E7)
	except:
		print 'error '
...
fig.savefig('/Users/chrisfuller/Dropbox/phd/papers/coma/smass_Fucntion.pdf')
plt.show()
