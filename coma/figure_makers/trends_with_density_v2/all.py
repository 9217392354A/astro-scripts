...
cat.add_column('METAL_STARS', cat.zGas - cat.SMASS)
...
if True:
	...
	ycols = ['DUST_STARS', 'sSFR', 'G2S','SMASS']
...
labels = {	'DMASS' 		: '$\log_{10} (M_{dust} / $M$_{\odot}$)', 
			...
			'METAL_STARS'	: '$\log_{10}$(Z/$M_{star}) (M_{\odot})$',
...
fig, subs = plt.subplots(nrows=len(ycols), ncols=len(xcols), sharex=False, sharey=False, squeeze=False, figsize = (8., 8.0), facecolor='w',edgecolor='w')
...
##### plotting options
plt.subplots_adjust(left=0.15, bottom=0.08, right=0.97, top=0.97, wspace=0.0, hspace=0.0)
fig.savefig('/Users/chrisfuller/Dropbox/twd.pdf')
plt.show()
