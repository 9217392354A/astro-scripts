...
if True:
	...
	ycols = ['colour', 'SRF', 'sSFR', 'METAL', 'SMASS']
...
fig, subs = plt.subplots(nrows=len(ycols), ncols=len(xcols), sharex=False, sharey=False, squeeze=False, figsize = (8., 9.5), facecolor='w',edgecolor='w')
...
for j in range(len(ycols)):
	...
	for i in range(len(xcols)):
		...
		#label y on final x
		if i == 0:
			sub.set_ylabel(labels[col])#,fontsize=10)
...
##### plotting options
plt.subplots_adjust(left=0.11, bottom=0.06, right=0.97, top=0.97, wspace=0.0, hspace=0.0)
fig.savefig('/Users/chrisfuller/Dropbox/phd/papers/coma/sdss-density-v2.pdf')
plt.show()
