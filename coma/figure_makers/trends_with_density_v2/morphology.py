...
fig, subs = plt.subplots(nrows=1, ncols=len(xcols), sharex=False, sharey=True, squeeze=False, figsize = (8., 2.), facecolor='w',edgecolor='w')
...
def fit_line2(x_data , y_data, y_error):
	...
	#m, dm  = out.params['m'].value, out.params['m'].stderr
	return x_data , model2(out.params, x_data), [-100., -100.0, -100., -100.0], out.redchi
...
def lineFitter(x_data, y_data, y_error):
	...
	if p_lin < tolerence:
		...
		info_2 = x_data , model2(out_poly2.params, x_data), [-100., -100.,-100., -100.0], [p_poly2, chi_poly2, nfree_poly2]
		if p_poly2 < tolerence and nfree_poly2 > 3:
			return info_2
...
def printTable(dict):
	...
	for j in range(len(ycols)):
		...
		printCoeff(dict, ycols[j]) 

def printCoeff(dict, ycol): #coeffs_dict[density+types[k]+col+ll] = coeffs #dic to hold grads
...
for j in range(len(ycols)):
	...
	for i in range(len(xcols)):
		...
		sub = subs[0,i]
		bins_density = np.linspace(np.floor(np.min(cat[density])), np.ceil(np.max(cat[density])), N_den+1) #automatic

		#extract raw data
		x_raw, y_raw = cat[density], cat[col]
		...
		sub.errorbar(x, y, yerr=y_err, c='k', ls='', alpha=0.35)
		sub.scatter(x, y, marker='o', s=y_n*2.0, c='k', edgecolor='k', alpha=0.35)
		...
		if len(x) > 3:
			...
			sub.plot(x_line, y_line, c='k', ls='-', lw=1, alpha=1.)

		else:
			coeffs = [-100.0, -100.0]

		coeffs_dict[density+col] = coeffs #dic to hold grads
...
##### plotting options
plt.subplots_adjust(left=0.11, bottom=0.25, right=0.97, top=0.95, wspace=0.0, hspace=0.0)
fig.savefig('/Users/chrisfuller/Dropbox/phd/papers/coma/morph-density-v3.pdf')
plt.show()
