...
inputs = ['cat', 'cat']
catNames = ['Coma Cluster', 'Coma Filament']
...
def lmfitter(x , y, y_error):
	params = Parameters()
	params.add('phiStar', value=0.457, vary=True)
	params.add('LStar', value=0.004, vary=True)
	params.add('alpha', value=-0.534, vary=True)
...
def printFit(params):
	...
	try:
		myPrint( 'alpha:', alpha, phiStarErr)
		myPrint( 'phiStar:', phiStar, LStarErr)
		myPrint( 'LStar:', LStar, alphaErr)
	except:
		print 'error '
...
plt.subplots_adjust(left=0.15, bottom=0.13, right=0.97, top=0.97, wspace=0.3, hspace=0.3)
fig.savefig('/Users/chrisfuller/Dropbox/phd/papers/coma/paras_Fucntion.pdf')
plt.show()
