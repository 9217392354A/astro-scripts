...
for j in range(len(cols)):
	...
	for ll in range(2):
		...
		x_raw = sub_cat.my_morph
		y_raw = sub_cat[cols[j]]

		#find mean and errors for each point
		x, y, y_err, y_n, y_std = mean_binup(x_raw, y_raw, np.linspace(-1.0, 1.0, 6))
		sub.scatter(x_raw, y_raw, s=40, marker=mkrs[ll], facecolor='k', edgecolor='k', alpha=0.25)

		sub.plot(x, y+y_std, ls = '--', c=colours[ll])
		sub.plot(x, y-y_std, ls = '--', c=colours[ll])

		sub.errorbar(x, y, yerr=y_err, c=colours[ll], ls='-')
		sub.scatter(x, y, marker='o', s=80, c=colours[ll], edgecolor='None', alpha=0.9)
...
sub.set_xlabel('$p(S)-p(E)$')
plt.subplots_adjust(left=0.12, bottom=0.05, right=0.99, top=0.99, wspace=0.0, hspace=0.0)
fig.savefig('/Users/chrisfuller/Dropbox/phd/papers/coma/hubble_sequence-phi.pdf')
plt.show()
