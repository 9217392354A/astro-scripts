...
def plotta(band,litfluxs,dX,dY,x,y,tune):
   ...
   # for tick in #r.yaxis.get_major_ticks():
    ...
    for litflux in listfluxs:
        w = np.where((litflux > 0.0) & (hefocs > 0.0))[0]
        litflux_c,hefocs_c = litflux[w],hefocs[w]
...
def plotta_sub(band,litfluxs,i):
    ...
    if i == 2: #label 
        ...
        f.set_ylabel("Paper Flux (Jy)",size=10)
...
fb = [["F500_NGP"],["F350_NGP"],["F250_NGP"], ['F160_DEEP'],['F100_DEEP', 'F100_IRAS-POINT', 'F100_IRAS-FAINT']]
xxvals = [1,3,5,7,9]
for i in range(0,len(bands)):

    plotta_sub(bands[i], fb[i], i) 
...
#plt.savefig('/Users/chrisfuller/Desktop/pdfs/flux_verfication.pdf')
plt.show()