...
cat = at.Table("/Users/chrisfuller/Dropbox/phd/herchel/coma/final_outputs/coma_supercluster_cal12.fits")
...
for i in range(1,len(bands)+1):
    ...
    f1.text(0.02, 0.9, bands[i-1][-3:]+"$\mu m$", transform=f1.transAxes, fontsize=15, verticalalignment='top')
    print bands[i-1], od_y

f1.set_xlabel("$m_{bt}$")
f1.tick_params(axis='both',labelleft='on', labelbottom='on')

plt.subplots_adjust(left=0.16, bottom=0.06, right=0.98, top=0.992, wspace=0.0, hspace=0.0)
