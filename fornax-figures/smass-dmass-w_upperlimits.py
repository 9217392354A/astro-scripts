...
def flux2mass(f):
	return 0.78937274*np.log10(f)+6.48619262
...
def myplot(can,morph_select, morph, dmass, what_dust, stars, dust_type,marker, colour):
	...
	try: print 'range: ', np.min(Y), '<--->', np.max(Y), ' maker and colour:', marker, colour, ' morph:',morph_select
	except: print 'error..'
...
#virgo dwarf
myplot(sub1,dwarf,virgo['goldmine'], virgo_all_dmass, what_dust_virgo, virgo_smass, 1,'o','red')
#myplot(sub1,dwarf,virgo['goldmine'], virgo_all_dmass, what_dust_virgo, virgo_smass, 2,'+','yellow')
#myplot(sub1,dwarf,virgo['goldmine'], virgo_all_dmass, what_dust_virgo, virgo_smass, 3,'v','yellow')
...
#virgo later
myplot(sub1,later,virgo['goldmine'], virgo_all_dmass, what_dust_virgo, virgo_smass, 1,'o','blue')
#myplot(sub1,later,virgo['goldmine'], virgo_all_dmass, what_dust_virgo, virgo_smass, 2,'o','blue')
#myplot(sub1,later,virgo['goldmine'], virgo_all_dmass, what_dust_virgo, virgo_smass, 3,'v','blue')

print 'Fornax Dwarf'
#plot a squar round all fornax galaxies
#fornax dwaro
myplot(sub1,dwarf,fornax['goldmine'], fornax_all_dmass, what_dust_fornax, fornax_smass, 1,'o','red')
myplot(sub1,dwarf,fornax['goldmine'], fornax_all_dmass, what_dust_fornax, fornax_smass, 2,'+','red')
#myplot(sub1,dwarf,fornax['goldmine'], fornax_all_dmass, what_dust_fornax, fornax_smass, 3,'v','yellow')
...
print 'Fornax Later'
#fornax later
myplot(sub1,later,fornax['goldmine'], fornax_all_dmass, what_dust_fornax, fornax_smass, 1,'o','blue')
myplot(sub1,later,fornax['goldmine'], fornax_all_dmass, what_dust_fornax, fornax_smass, 2,'+','blue')
#myplot(sub1,later,fornax['goldmine'], fornax_all_dmass, what_dust_fornax, fornax_smass, 3,'v','green')#plot vertical line at stellar mass of 8
#sub1.axvline(x=8.5, color='k', ls='--')
#sub1.axhline(y=4.5, color='k', ls='-')


#draw line to represent min dust mass that could be detected
XXX = np.arange(1,18.,0.1)
sub1.plot(XXX,5.04925395366-XXX,'k--')
...
sub1.set_xlabel('$\log_{10}$$(M_{star} / M_{\odot})$')
sub1.set_ylabel('$\log_{10}$$(M_{dust} / M_{stars})$')
plt.subplots_adjust(left=0.14, bottom=0.11, right=0.95, top=0.95, wspace=0.0, hspace=0.0)
#figM.savefig(pj('/Users/chrisfuller/Dropbox/phd/papers/fornax','smass_vs_dmass.pdf'),dpi=600)
plt.show()
