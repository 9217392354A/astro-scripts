...
cat_mine = at.Table(pj(folder, 'HeFoCS-fluxes-260313-mybgsub-v2.fits'),type='fits')
catA = at.Table(pj(folder, '1.24_hefocs_beam.fits'),type='fits')


#key parameters
r_search = 14.1 / 3600.0 # search radius in degrees
coor_names_A = ['GRA2000', 'GDEC2000'] # these are the colum names that containe ra and dec
coor_names_B = ['ALPHA_J2000', 'DELTA_J2000']
n=1000#number of interations of monty carlo
n_bins = 4 #number of radial bins


print 'bin width = ', r_search*3600.0/n_bins
# # # # # # # # # # # # # # # Function # # # # # # # # # # # # # # # # # # # # # # # # # #
...
for i in range(0,n):
	...
	rand_cat = random_cat(54.4, -35.4, 7.0, len(cat_mine)*2)
	new_cat = x_matcher(rand_cat, ['ra_rand', 'dec_rand'], catA, coor_names_B , r_search )
...
new_cat_mine = x_matcher(cat_mine, ['GRA2000', 'GDEC2000'], catA, coor_names_B , r_search )
#list of seps
#find sources with matches
...
fig = plt.figure(figsize = (4.6,9.6),facecolor='w',edgecolor='w')
...
sub2.bar(binedges[:-1], conta_hist_fraction*100.0, width=r_search*3600.0/n_bins, fc ='b')
#sub2.plot(X_con, Y_con*100.0, 'k--')
sub2.set_xlabel('Angular Separation, (arcsec)')
sub2.set_ylabel('Contamination (%)')
sub2.set_ylim(ymin=0, ymax=60)

sub1.set_ylim(ymax=14)
sub1.set_xlim(xmin=0, xmax=binedges[-1])
sub2.set_xlim(xmin=0, xmax=binedges[-1])
plt.subplots_adjust(left=0.13, bottom=0.06, right=0.97, top=0.98,hspace=0.0)
