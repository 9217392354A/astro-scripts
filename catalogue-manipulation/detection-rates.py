...
for catName in cats:
    ...
    for band in bands:
        ...
        for i in range(0,len(cat)):
            if cat.HEVICS_PLW[i] == 3 and band == "F100": cat[band][i]= -1.0
            if cat.HEVICS_PLW[i] == 3 and band == "F160": cat[band][i]= -1.0            
            #get flux
            try: 
                flux = float(cat[band][i])
                if flux == 0.0: notDet +=1      # not detected 
                elif flux == -1.0: notMeasured += 1
                else: Det +=1                    # detected
            except:
                notMeasured += 1                 # not measured
        total = Det + notDet
