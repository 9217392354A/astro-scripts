...
cat = at.Table(pj(folder,"flux_verfication-all.fits"),type='fits')
...
fig = plt.figure(figsize = (5.6,9.6),facecolor='w',edgecolor='w')
...
for j in range(0,len(fa)):
	...
	for i in range(0,len(fb[j])):
		...
		if j != 4:
			...
			#for tick in sub.xaxis.get_major_ticks(): tick.label.set_fontsize(6)
			#for tick in sub.yaxis.get_major_ticks(): tick.label.set_fontsize(6) 


	sub.set_xlim(xmin=10.0**-2., xmax=10**1.2 )
	sub.set_ylim(ymin=10.0**-2., ymax=10**1.2 )

	sub.text(0.05, 0.95, names[j], transform=sub.transAxes, fontsize=14, verticalalignment='top')

plt.subplots_adjust(left=0.115, bottom=0.06, right=0.98, top=0.98, wspace=0.0, hspace=0.0)
