...
#file stuff
folder = "/Users/chrisfuller/Dropbox/phd/herchel/fornax/SPIRE-contamination/"
allfile = 'cat.csv'
...
#cat = np.loadtxt(pj(folder,allfile), dtype=str,  skiprows=1, delimiter=",", unpack=False)
#catHead = np.loadtxt(pj(folder,allfile), dtype=str,  skiprows=0, delimiter=",", unpack=False)[0,:]
badgals = np.loadtxt(pj(folder,badgal), dtype=str)

cat = at.Table("/Users/chrisfuller/Dropbox/phd/herchel/fornax/final_outputs/HeFoCS-new_spire.fits",type='fits')
...
def writeheader(folder,filename):
    ...
    print '\\hline'
    print 'Flux bin & Na & Nr & Nu & Np & contamination & Nc \\\\ '
...
def saveResult(band,bins,Nr,Na,Np, Nu, cont, Nc, folder,filename):
    ...
    for i in range(0,len(Nr)):
        if i == len(Nr) -1:
            x = '~' + '&' + s(int(Na[i])) + '&' + s(int(Nr[i])) + '&' + s(int(Nu)) + '&' + s(int(Np[i])) + '&' + s(cont[i]) + '&' + s(int(Nc[i])) + '\\\\ '
        else:
            x = '~' + '&' + s(int(Na[i]))+ '&' + s(Nu)  + '&' + s(int(Nr[i])) + '&' + s(int(Np[i])) + '&' + s(cont[i]) + '&' + s(int(Nc[i])) + '\\\\'
        print x
    outfile.close()
...
for band in bands:  
    ...
    if band == 250:
        ...
        Np = np.array([8502./20.])#+64.35+1.1])
    #now cycle through each galaxies only intrested in the point population
